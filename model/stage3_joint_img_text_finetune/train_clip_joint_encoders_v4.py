...
class IdentityPromptDataset(torch.utils.data.Dataset):
    def __init__(self, image_dir, image_meta_path, identity_prompts_path, transform, max_samples=None, seed=42,
                 split="train"):
...
def train(args):
    ...
    print(
        f"Dataset: {len(dataset)} | BS: {args.p * args.k} | Epochs: {args.epochs} | LR: {args.lr} | Loss: {args.loss}")
